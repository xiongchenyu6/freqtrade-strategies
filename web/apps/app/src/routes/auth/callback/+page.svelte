<script lang="ts">
	...
	onMount(async () => {
		try {
			const { next } = await handleAuthCallback();
			// eslint-disable-next-line svelte/no-navigation-without-resolve -- runtime path, same-origin-checked in handleAuthCallback
...
<main class="mx-auto mt-32 max-w-md px-5 text-center">
	{#if status === 'loading'}
		<div
			class="mx-auto h-8 w-8 animate-spin rounded-full border-2 border-muted border-t-primary"
		></div>
	...
	{:else}
		...
		<p class="mt-4 text-sm">
			<a href={resolve('/login')} class="text-primary hover:underline">← {t(lang, 'login.title')}</a
			>
		</p>
	{/if}
</main>
