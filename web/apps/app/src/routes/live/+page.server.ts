...
export const load: PageServerLoad = async ({ fetch, cookies }) => {
	...
	const [runs, trades, events, closedTrades, nautilusTrades] = await Promise.all([
		...
		vps.nautilusTrades(fetch, { limit: 100, authHeader: auth }).catch(() => []),
	]);
	return { runs, trades, events, closedTrades, nautilusTrades };
};
