<script lang="ts">
	...
	const merged = $derived.by((): SignalItem[] => {
		...
		return all.filter((s) => {
			if (s._type === 'event') {
				...
				const r = s.item;
				return (
					r.strategy.toLowerCase().includes(q) || (r.timeframe ?? '').toLowerCase().includes(q)
				);
			}
		});
	});
	...
	const signalKindTimeline = $derived.by(() => {
		...
		const maxTotal = Math.max(
			1,
			...weeks.map(([, k]) => KINDS.reduce((s, kk) => s + (k[kk] ?? 0), 0))
		);
	...
	const fngHistogram = $derived.by(() => {
		...
		return {
			buckets: buckets.map((b) => ({ ...b, barPct: (b.count / maxCount) * 100 })),
			avg,
			total: vals.length
		};
	});
	...
	const dcaCumulativeSpend = $derived.by(() => {
		...
		let running = 0;
		const pts = evts.map((e, i) => {
			running += e.amount_usdt!;
			return { i, total: running, date: e.ts.slice(0, 10), kind: e.kind };
		});
	...
	const fngZoneEventCount = $derived.by(() => {
		...
		const rows = ZONES.map((z) => {
			...
			const avgAmt = withAmt.length
				? withAmt.reduce((s, e) => s + e.amount_usdt!, 0) / withAmt.length
				: null;
...
<main class="mx-auto w-full max-w-[1600px] px-4 py-8 sm:px-6">
	...
	{#if !isGated}
		<div class="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
			<div class="rounded-lg border border-border bg-card px-4 py-3">
				<div class="text-[11px] text-muted-foreground uppercase">DCA Events</div>
				...
				<div class="mt-1 flex flex-wrap gap-1">
					{#each Object.entries(stats.kindCounts) as [kind, n]}
						<span
							class="rounded px-1.5 py-0.5 text-[10px] font-bold {KIND_BADGE[kind] ??
								'bg-secondary text-muted-foreground'}">{kind} {n}</span
						>
					{/each}
				</div>
			</div>
			...
			<div class="rounded-lg border border-border bg-card px-4 py-3">
				<div class="text-[11px] text-muted-foreground uppercase">Total Signals</div>
				<div class="mt-1 font-mono text-lg font-semibold">
					{stats.totalEvents + stats.totalRuns}
				</div>
			</div>
			<div class="rounded-lg border border-border bg-card px-4 py-3">
		...
		<ul class="space-y-3">
			{#each merged as signal (signal._ts + signal._type + (signal._type === 'event' ? signal.item.kind : signal.item.id))}
				{#if signal._type === 'event'}
					...
					<li class="rounded-xl border border-l-4 border-border bg-card {borderCls} px-4 py-3">
						<!-- Single-row on lg+, wrap groups on smaller screens. -->
						<div class="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm">
							<span class="shrink-0 rounded-full px-2 py-0.5 text-xs font-bold {badgeCls}"
								>{e.kind}</span
							>
							<span class="shrink-0 text-xs text-muted-foreground tabular-nums"
								>{fmtTime(e.ts)}</span
							>
							<span class="shrink-0 font-medium">BTC {fmtUSD(e.price)}</span>
							{#if e.fng != null}
								<span class="shrink-0 text-xs text-muted-foreground"
									>FnG <span class="font-mono text-foreground">{e.fng}</span></span
								>
							{/if}
							{#if e.severity != null}
								<span class="shrink-0 text-xs text-muted-foreground">
							...
							{#if e.amount_usdt != null}
								<span class="shrink-0 text-xs text-muted-foreground"
									>{lang === 'zh' ? '金额' : 'Amount'}
									<span class="font-mono text-foreground">{fmtUSD(e.amount_usdt)} USDT</span></span
								>
							{/if}
							{#if e.mode}
								<span class="shrink-0 text-xs text-muted-foreground"
									>mode <span class="font-mono text-foreground">{e.mode}</span></span
								>
							{/if}
							<span
								class="ml-auto shrink-0 rounded-full border border-orange-900 bg-orange-950/60 px-2 py-0.5 text-[10px] font-semibold text-orange-400"
								>EVENT DCA</span
							>
						</div>
					</li>
	...
	{#if eventCalendar}
		<section class="mt-8 rounded-lg border bg-card p-4">
			<h2 class="mb-1 text-sm font-semibold">
				{lang === 'en' ? 'Which days had signals?' : '哪些天有信号?'}
				<ChartInfo metric="dcaTrigger" {lang} />
			</h2>
			<p class="mb-3 text-[10px] text-muted-foreground">
				30-Day Event Activity · DCA triggers · last 30 days
			</p>
			<div class="space-y-1.5">
				{#each eventCalendar.rows as row}
					<div class="grid grid-cols-6 gap-1.5">
						{#each row as cell}
							{@const alpha =
								cell.count === 0 ? 0 : 0.15 + (cell.count / eventCalendar.maxCount) * 0.75}
							<div
								class="flex h-10 flex-col items-center justify-center rounded font-mono text-[10px] leading-tight"
								style="background:rgba(99,102,241,{alpha.toFixed(2)})"
								title="{cell.date}: {cell.count} event{cell.count !== 1 ? 's' : ''}{Object.keys(
									cell.kinds
								).length
									? ' · ' +
										Object.entries(cell.kinds)
											.map(([k, v]) => k + ':' + v)
											.join(', ')
									: ''}"
							>
			...
			<p class="mt-2 text-[10px] text-muted-foreground">
				Each cell = 1 day · indigo intensity ∝ event count
			</p>
		</section>
	{/if}
	...
	{#if dcaCumulativeSpend}
		...
		<section class="mt-6 rounded-lg border bg-card p-4">
			<h2 class="mb-1 text-sm font-semibold">
				{lang === 'en' ? 'How much have the signals bought?' : '信号累计买入了多少?'}
				<ChartInfo metric="dcaTrigger" {lang} />
			</h2>
			<p class="mb-3 text-[10px] text-muted-foreground">
				Cumulative DCA Spend · {dcs.n} events · {dcs.firstDate} → {dcs.lastDate}
			</p>
			<svg viewBox="0 0 {dcs.W} {dcs.H}" class="w-full" style="height:{dcs.H}px">
				<polyline
					points={dcs.polyline}
					fill="none"
					stroke="var(--ch-violet-strong)"
					stroke-width="1.5"
					stroke-linejoin="round"
				/>
				...
				<text
					...
					font-size="7"
					fill="var(--ch-rule)"
					text-anchor="end">{dcs.lastDate}</text
				>
				<text
					x={dcs.W - dcs.PAD}
					y="10"
					font-size="8"
					fill="var(--ch-violet-strong)"
					text-anchor="end">${dcs.total.toFixed(0)} total</text
				>
			</svg>
			<p class="mt-1 text-[10px] text-muted-foreground">
				Running cumulative USDT deployed across all DCA trigger events · total deployed: ${dcs.total.toFixed(
					0
				)}
			</p>
		</section>
		...
		<div class="space-y-8 p-4 pt-0">
			...
			{#if severityTrend}
				...
				<section class="rounded-lg border bg-card p-4">
					<div class="mb-2 flex items-baseline justify-between">
						<h2 class="text-sm font-semibold">
							Market Stress Trend <span class="ml-1 text-xs font-normal text-muted-foreground"
								>(7-event rolling avg severity)</span
							>
							<ChartInfo metric="fearGreed" {lang} />
						</h2>
						<span
							class="font-mono text-xs {st.rising
								? 'text-red-400'
								: st.falling
									? 'text-green-400'
						...
						>
							{st.latest.toFixed(3)}
							{st.rising ? '↑ rising' : st.falling ? '↓ easing' : '→ stable'}
						</span>
					</div>
					<svg viewBox="0 0 {st.W} {st.H}" class="w-full" style="height:{st.H}px;min-width:200px">
						...
						<text
							...
							y={st.H - 2}
							font-size="7"
							fill="var(--ch-rule-strong)"
							text-anchor="end">latest</text
						>
					</svg>
					<p class="mt-1 text-[10px] text-muted-foreground">
						Severity 0 = mild signal · 1 = extreme · red = stress rising · green = stress easing
					</p>
				</section>
			{/if}
			...
			{#if signalKindTimeline}
				...
				<section class="rounded-lg border bg-card p-4">
					<h2 class="mb-3 text-sm font-semibold">
						Weekly Signal Mix <span class="ml-1 text-xs font-normal text-muted-foreground"
							>(last 8 weeks · stacked by kind)</span
						>
						<ChartInfo metric="dcaTrigger" {lang} />
					</h2>
					<div class="flex items-end gap-1">
						{#each skt.weeks as [week, kinds]}
							...
							<div class="flex flex-1 flex-col items-center gap-1">
								<div
									class="relative flex w-full flex-col-reverse overflow-hidden rounded-t-sm"
								...
								>
									{#each skt.KINDS as k}
										{#if (kinds[k] ?? 0) > 0}
											...
											<div
												style="height:{Math.max(1, segH)}px;background:{KIND_COLORS[k] ??
													'var(--ch-axis-muted)'}"
												title="{k}: {kinds[k]}"
											></div>
										{/if}
								...
								<span
									class="origin-center rotate-[-45deg] font-mono text-[8px] text-muted-foreground"
									>{week.slice(5)}</span
								>
							</div>
						{/each}
			...
			{#if fngHistogram}
				...
				<section class="rounded-lg border bg-card p-4">
					<div class="mb-3 flex items-baseline justify-between">
						<h2 class="text-sm font-semibold">
							Fear &amp; Greed at Trigger Time <span
								class="ml-1 text-xs font-normal text-muted-foreground"
								>({fh.total} events with FNG data)</span
							>
							<ChartInfo metric="fearGreed" {lang} />
						</h2>
					...
					<div class="flex items-end gap-1">
						{#each fh.buckets as b}
							<div
								...
								title="{b.lo}-{b.lo + 10}: {b.count} events"
							>
								<div
									class="w-full rounded-t-sm"
									style="height:{Math.max(1, Math.round(b.barPct * 0.6))}px; background:{b.color}"
								></div>
								<span class="text-center font-mono text-[7px] text-muted-foreground">{b.label}</span
								>
							</div>
						{/each}
					</div>
					<p class="mt-2 text-[10px] text-muted-foreground">
						Green = fear zone (&lt;30) · amber = neutral · red = greed zone (&gt;70) · x-axis = FNG
						index 0→100
					</p>
				</section>
			{/if}
			...
			{#if fngZoneEventCount}
				<section class="rounded-lg border bg-card p-5">
					<h2 class="mb-3 text-sm font-semibold">
						Events by Fear & Greed Zone
						<span class="ml-1 text-xs font-normal text-muted-foreground"
							>(count + avg USDT deployed per sentiment zone)</span
						>
						<ChartInfo metric="fearGreed" {lang} />
					</h2>
					<div class="space-y-2">
						{#each fngZoneEventCount as z}
							<div class="flex items-center gap-3">
								...
								<div class="relative h-5 flex-1 overflow-hidden rounded-sm bg-muted/20">
									<div
										class="absolute inset-y-0 left-0 rounded-sm transition-all"
										style="width:{z.barPct.toFixed(1)}%; background:{z.color}"
									></div>
					...
					<p class="mt-2 text-[10px] text-muted-foreground">
						Bar = event count · right = avg USDT deployed · green = fear zones (buy signals) · red =
						greed zones
					</p>
				</section>
			{/if}
